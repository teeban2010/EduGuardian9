...
function ExploreItem({ icon, label, color, bgColor, onClick }: ExploreItemProps) {
  return (
    <Box
      onClick={onClick}
      sx={{
        ...
        '&:hover': { transform: 'translateY(-2px)', bgcolor: alpha(color, 0.06) },
      }}
    >
      ...
      <Typography variant="caption" fontWeight={600} textAlign="center" sx={{ lineHeight: 1.2 }}>{label}</Typography>
    </Box>
  );
}
...
export default function ParentDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: s } = await supabase.from('students').select('*').eq('school_id', school.id).order('created_at');
  ...
  const firstName = profile?.full_name ?? 'Parent';

  const exploreItems: ExploreItemProps[] = [
    { icon: <EventNoteIcon />, label: 'Attendance', color: '#2563EB', bgColor: alpha('#2563EB', 0.12), onClick: () => navigate('/attendance') },
    { icon: <AssignmentIcon />, label: 'Homework', color: '#10B981', bgColor: alpha('#10B981', 0.12), onClick: () => navigate('/homework') },
    { icon: <ShieldIcon />, label: 'Discipline', color: '#EF4444', bgColor: alpha('#EF4444', 0.12), onClick: () => navigate('/discipline') },
    { icon: <TrendingUpIcon />, label: 'Academic\nProgress', color: '#8B5CF6', bgColor: alpha('#8B5CF6', 0.12), onClick: () => navigate('/progress') },
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Avatar
                  ...
                  sx={{
                    ...
                    fontSize: '1.4rem', border: '3px solid', borderColor: 'primary.light',
                    flexShrink: 0,
                  }}
                >
                  {activeStudent?.full_name?.[0] ?? 'A'}
                </Avatar>
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <Typography variant="subtitle1" fontWeight={700} noWrap>
                    {loading ? 'Loading...' : activeStudent?.full_name ?? 'No child added'}
                  </Typography>
                  <Typography variant="body2" color="text.secondary" noWrap>
                    {activeStudent?.class_name ?? 'Class not set'}
                  </Typography>
                ...
                <Button
                  ...
                  sx={{
                    color: 'primary.main', fontWeight: 600, fontSize: '0.75rem',
        ...
        <Grid size={12}>
          <Card
            sx={{
              ...
              cursor: 'pointer',
              '&:hover': { boxShadow: `0 4px 20px ${alpha('#2563EB', 0.15)}` },
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Typography variant="body2" fontWeight={500}>
                    {activeStudent?.full_name?.split(' ')[0] ?? 'Your child'} is doing well! {'\u{1F604}'}
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
          ...
          <Card sx={{ borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
            <CardContent sx={{ p: 2.5 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                ...
                <Button size="small" endIcon={<ArrowForwardIosIcon sx={{ fontSize: '11px !important' }} />}
              ...
              <Grid size={3} key={item.label}>
                ...
                >
                  <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
                    ...
                    <Typography variant="subtitle2" fontWeight={800} sx={{ color: item.color, mt: 0.25 }}>
                      {item.value}
                    </Typography>
                    {item.sub && (
              ...
              {UPCOMING_EVENTS.map((ev, i) => (
                <Box key={i}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, px: 2.5, py: 1.75 }}>
                    <Box
                      sx={{
                        width: 40, height: 40, borderRadius: 2, bgcolor: alpha(ev.color, 0.12),
        ...
        <Grid size={12}>
          <Card
            sx={{
              ...
              cursor: 'pointer',
              '&:hover': { boxShadow: `0 6px 24px ${alpha('#EF4444', 0.4)}`, transform: 'translateY(-1px)' },
          ...
          >
            <CardContent sx={{ p: 2.5 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Box
                  sx={{
                    width: 44, height: 44, borderRadius: '50%', bgcolor: 'rgba(255,255,255,0.2)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                  }}
                >
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.8)' }}>
                    Tap to alert the school & your contacts
                  </Typography>
                </Box>
